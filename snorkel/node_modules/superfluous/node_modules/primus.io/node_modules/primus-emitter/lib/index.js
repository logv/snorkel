...
function PrimusEmitter(primus) {
  ...
  primus.send = function send(ev, data, fn) {
    ...
    // if primus-rooms is present then we attempt to broadcast
    // if result is negative then it means this is not a broadcast
    // event so we execute a regular send.
    if ('rooms' in primus.$) {
      var sparks = primus.connections;
      if (primus._rooms.broadcast(primus, args, sparks, null, 'send')) {
        return this;
      }
    }
