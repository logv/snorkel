// TODO: figure out how to scope modules depending on who required them.
// Deal with module.ids no longer being unique. Etc.
// Best solution could be to have modulr's dev runtime's require function
// scoped by package (and thus with a notion of path) and the prod env use md5 hashes
// as ids and thus remove all duplicates that way.
...
/*
  ...
  If it finds a descriptor file, it will extract from it data
  necessary to create a package-scoped src resolver.
  
  It will then proceed to look for the relative path to the lib
...
(function(p) {
    ...
    this.getPackageForModule(module, function(err, pkg) {
      if (err) {
        ...
        if (module.id === pkg.id) {
          ...
          relativePath = path.join(pkg.lib, identifiers);
        }
        self.getScopedResolver(pkg).resolvePath(relativePath, module, function(err, isDir) {
          if (err) {
            callback(err, isDir);
          } else {
  ...
  p.getScopedResolver = getScopedResolver;
  function getScopedResolver(pkg) {
    var resolvers = this.resolvers,
        id = pkg.id;
    if (!(id in resolvers)) {
      resolvers[id] = createSrcResolver({
        allowDirModules: pkg.allowDirModules || this.allowDirModules,
        paths: pkg.paths || this.paths,
        root: pkg.fullPath,
        ext: pkg.extensions || this.extensions,
      });
    }
    return resolvers[id];
  }
